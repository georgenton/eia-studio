...
| 3 | **Field / survey foundation** | SurveyTemplate/Version/Question, SurveyInstance/Answer (immutable), Visit, Media (presigned upload), Assignment, Device, PII schema and `pii.read` policy, Field Surveys inbox with validation, Parcel tabs Visitas/Instrumentos/Media, survey import (CSV/Kobo later). **D-020 settled here (ADR-018):** offline capture is the configuration `field.surveys.offline_mode = disabled / optional / required`, not a capability; the catalogue stays at fourteen keys | 0, 1, 2 |
