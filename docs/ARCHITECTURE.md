...
- Schemas: `app` (operational), `pii` (identified personal data, stricter grants), `portal`
  (publication projections, read by the portal role), `audit` (append-only). `vec` (chunk
  embeddings) is **not created**: Slice 6 ships full-text retrieval and no embedding path at all
  (ADR-021); the schema arrives with the semantic adapter, under the same RLS predicates.
...
| Vector / RAG | **Amended by ADR-021 (Slice 6): retrieval ships as PostgreSQL full-text** (a generated `tsvector` on `document_chunk`, GIN index, `spanish` configuration) behind a `DocumentRetriever` port. pgvector remains the intended semantic store and is **not installed**: no embedding provider is configured anywhere, and a vector column filled by a deterministic stand-in is indistinguishable from one a model produced — the artefact IG4-001 exists to prevent. There is no `vec` schema and no embedding column. | Pinecone, Qdrant, Weaviate; pgvector with a fake embedder; both stacks hybrid-ranked | Isolation is inherited from RLS either way. Adding the semantic adapter later is an added implementation of the existing port, not a rewrite; recall is lexical until then (TD-057). |
