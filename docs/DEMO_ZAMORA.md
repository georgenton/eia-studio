...
| Visited, revisits, pending, productivity, projected close, activity feed, field inbox | Operational simulation to demonstrate the workflow, counted from **the current campaign** (ADR-026); a closed one is history and takes no part | DEMO_SIMULATION | SYSTEM_GENERATED | [ORIGINAL] or [DERIVED] | AGGREGATE | Simulación operativa |
