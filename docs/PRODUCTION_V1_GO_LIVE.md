...
| 6 | **A correction workflow** (TD-114) | A submitted response is immutable by design, and so now are a photograph's declaration (TD-097) and a document version. Eight projects will produce corrections, and there is no reviewed path for one (TD-060 is the same gap for reports). Writing the *questionnaire* is now a product act (blocker 8); correcting an *answer* is not, and it is a different problem: a superseding instance plus one resolver every analytic reads | dev |
