...
| Correction | a submitted response is never edited; correcting one is a **new capture** whose relation to the original is a row (ADR-038). Asking for one is `field.corrections.request`, held by the two roles that already read a response; reading the lineage is `field.responses.read`, because it is a statement about a household's answers. The **device is told the reason and no previous answer**, so a phone that would otherwise never hold another visit's responses does not start holding them because a figure was wrong |
